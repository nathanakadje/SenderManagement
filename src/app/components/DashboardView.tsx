...
export function DashboardView() {
  ...
  // Fonction pour générer les données mensuelles à partir des enregistrements réels
  const generateMonthlyData = async () => {
    try {
      const response = await fetch("http://localhost:3000/api/senders");
      if (!response.ok) throw new Error("Erreur lors de la récupération des senders");
      
      const senders = await response.json();
      
      // Initialiser les comptes pour chaque mois de l'année courante
      const monthlyCounts: Record<string, number> = {};
      const months = ["Jan", "Fév", "Mar", "Avr", "Mai", "Jun", "Jul", "Aoû", "Sep", "Oct", "Nov", "Déc"];
      months.forEach(month => { monthlyCounts[month] = 0; });
      
      // Compter les senders par mois pour l'année en cours
      const currentYear = new Date().getFullYear();
      
      senders.forEach((sender: any) => {
        if (sender.date_created) {
          const date = new Date(sender.date_created);
          const year = date.getFullYear();
          // Prendre uniquement les données de l'année en cours
          if (year === currentYear) {
      ...
      console.error("Erreur lors de la génération des données mensuelles:", err);
      // Retourner des données vides en cas d'erreur
      return [
        { month: "Jan", count: 0 }, { month: "Fév", count: 0 }, { month: "Mar", count: 0 },
        { month: "Avr", count: 0 }, { month: "Mai", count: 0 }, { month: "Jun", count: 0 },
        { month: "Jul", count: 0 }, { month: "Aoû", count: 0 }, { month: "Sep", count: 0 },
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Générer les données mensuelles réelles
        const realLineData = await generateMonthlyData();
        console.log("Données mensuelles générées:", realLineData);
        
        setDashboardData({
          kpis: {
            total: parseInt(data.kpiCards[0]?.value) || 0,
            validated: parseInt(data.kpiCards[1]?.value) || 0,
            pending: parseInt(data.kpiCards[2]?.value) || 0,
            rejected: parseInt(data.kpiCards[3]?.value) || 0
          },
          pieData: data.pieData || [],
          lineData: realLineData, // Utiliser les données réelles
  ...
  return (
    <div className="flex flex-col gap-5 p-6 overflow-auto h-full" style={{ background: "var(--background)" }}>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {/* Area Chart - Évolution mensuelle avec données réelles */}
        <Card className="p-5">
          <div className="flex items-center justify-between mb-4">
            <p style={{ color: "var(--foreground)", fontWeight: 700, fontSize: "0.9375rem" }}>Évolution mensuelle</p>
            <span className="px-2 py-0.5 rounded-full text-xs font-semibold" style={{ background: "var(--blue-muted)", color: "var(--primary)" }}>
              {new Date().getFullYear()}
            </span>
          </div>
          <ResponsiveContainer width="100%" height={210}>
            <AreaChart data={lineData} margin={{ top: 10, right: 5, left: -20, bottom: 0 }}>
              <defs>
        ...
        {/* Horizontal bars - Top Opérateurs */}
        <Card className="p-5">
          ...
          <div className="flex flex-col gap-4">
            {operatorData.slice(0, 5).map(({ operator, count }: any, i: number) => {
      ...
      {/* Recent Activities - 5 dernières activités */}
      <Card>
        <div
        ...
        <table className="w-full">
          ...
          <tbody>
            {recentActivities.slice(0, 5).map((row: any, i: number) => (
              <tr
                key={i}
