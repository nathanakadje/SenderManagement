...
import { ChevronDown, CheckCircle2, Search, X, AlertTriangle, Lock, Plus, ChevronRight } from "lucide-react";
...
// Sélection par pays : { [countryCode]: string[] }
type OperatorsByCountry = Record<string, string[]>;
...
export function SenderFormView({
  ...
  const [form, setForm] = useState<MultiFormState>({
    senderName: initialData?.senderName ?? "",
    countries: initialData?.country ? [initialData.country] : [],
    operatorsByCountry: initialData?.country && initialData?.operator
      ? { [initialData.country]: [initialData.operator] }
      : {},
  ...
  const [operatorOpen, setOperatorOpen]   = useState(false); // code pays ou ""
  ...
  const [addOperatorTarget, setAddOperatorTarget]       = useState<string>("");

  useEffect(() => { fetchCountries(); }, [refreshCountries]);
  ...
  const selectedCountryObjs = countries.filter((c) => form.countries.includes(c.code));
  ...
  // Nombre total de combinaisons à créer
  const combinations: { countryCode: string; countryName: string; operator: string }[] = [];
  for (const code of form.countries) {
    const obj = countries.find((c) => c.code === code);
    for (const op of form.operatorsByCountry[code] ?? []) {
      combinations.push({ countryCode: code, countryName: obj?.name ?? code, operator: op });
    }
  }

  /* ── Handlers ── */
  const toggleCountry = (code: string) => {
    const isSelected = form.countries.includes(code);
    if (isSelected) {
      const newCountries = form.countries.filter((c) => c !== code);
      const newOpsByCountry = { ...form.operatorsByCountry };
      delete newOpsByCountry[code];
      setForm({ ...form, countries: newCountries, operatorsByCountry: newOpsByCountry });
    } else {
      setForm({ ...form, countries: [...form.countries, code], operatorsByCountry: { ...form.operatorsByCountry, [code]: [] } });
    }
    if (errors.countries) setErrors({ ...errors, countries: undefined });
  };

  const toggleOperatorForCountry = (countryCode: string, op: string) => {
    const current = form.operatorsByCountry[countryCode] ?? [];
    const updated = current.includes(op)
      ? current.filter((o) => o !== op)
      : [...current, op];
    setForm({ ...form, operatorsByCountry: { ...form.operatorsByCountry, [countryCode]: updated } });
    if (errors.operators) setErrors({ ...errors, operators: undefined });
  };

  const selectAllOperatorsForCountry = (countryCode: string) => {
    const countryObj = countries.find((c) => c.code === countryCode);
    if (!countryObj) return;
    const current = form.operatorsByCountry[countryCode] ?? [];
    const allSelected = countryObj.operators.every((op) => current.includes(op));
    setForm({
      ...form,
      operatorsByCountry: {
        ...form.operatorsByCountry,
        [countryCode]: allSelected ? [] : [...countryObj.operators],
      },
    });
  };

  const addNewOperator = async () => {
    if (!newOperatorName.trim() || !addOperatorTarget) return;
    try {
      const res = await fetch(`http://localhost:3000/api/countries/${addOperatorTarget}/operators`, {
        method: "POST",
      ...
      if (res.ok) {
        ...
        const current = form.operatorsByCountry[addOperatorTarget] ?? [];
        setForm({ ...form, operatorsByCountry: { ...form.operatorsByCountry, [addOperatorTarget]: [...current, newOperatorName.trim()] } });
        setShowAddOperatorModal(false);
        setNewOperatorName("");
        setNotification({ type: "success", message: `Opérateur "${newOperatorName}" ajouté avec succès` });
      } else {
        setNotification({ type: "error", message: "Erreur lors de l'ajout de l'opérateur" });
      }
    } catch {
      setNotification({ type: "error", message: "Erreur de connexion lors de l'ajout de l'opérateur" });
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    const hasNoOperator = form.countries.some(
      (code) => (form.operatorsByCountry[code] ?? []).length === 0
    );
    if (hasNoOperator) {
      newErrors.operators = "Chaque pays doit avoir au moins un opérateur sélectionné";
    }
  ...
  const handleSubmit = async () => {
    ...
    if (onSubmit) {
      onSubmit({
        ...
        operator: (form.operatorsByCountry[form.countries[0]] ?? [])[0] ?? "",
        status: form.status,
    ...
    try {
      const results = await Promise.allSettled(
        combinations.map(({ countryName, operator }) =>
          fetch("http://localhost:3000/api/senders", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
      ...
      if (failed === 0) {
        setNotification({ type: "success", message: `${succeeded} enregistrement(s) créé(s) avec succès pour "${form.senderName}" !` });
        setForm({ senderName: "", countries: [], operatorsByCountry: {}, status: "", comment: "" });
        setErrors({});
        onSuccess?.();
        setTimeout(() => { onCancel?.(); }, 1000);
      } else if (succeeded > 0) {
        setNotification({ type: "error", message: `${succeeded} créé(s), ${failed} en erreur (doublons ?)` });
      } else {
        const reason = (results[0] as PromiseRejectedResult).reason;
        throw new Error(reason?.message ?? "Toutes les créations ont échoué");
      }
    } catch (err) {
      setNotification({ type: "error", message: err instanceof Error ? err.message : "Erreur lors de la création." });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleReset = () => {
    setForm({
      senderName: initialData?.senderName ?? "",
      countries: initialData?.country ? [initialData.country] : [],
      operatorsByCountry: initialData?.country && initialData?.operator ? { [initialData.country]: [initialData.operator] } : {},
      status: initialData?.status ?? "",
      comment: initialData?.comment ?? "",
    });
    setErrors({});
  ...
  const inputStyle: React.CSSProperties = {
    backgroundColor: "#ffffff", borderWidth: "1px", borderStyle: "solid",
    borderColor: "var(--border)", borderRadius: "8px", color: "var(--foreground)",
    fontSize: "0.875rem", outline: "none", transition: "border-color 0.15s, box-shadow 0.15s",
    width: "100%", padding: "0.625rem 0.875rem",
  };
  const errorInputStyle: React.CSSProperties = { ...inputStyle, borderColor: "var(--rose)", boxShadow: "0 0 0 3px rgba(225,29,72,0.1)" };
  const focusStyle = { borderColor: "var(--primary)", boxShadow: "0 0 0 3px rgba(37,99,235,0.1)" };

  const buttonStyle: React.CSSProperties = {
    width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between",
    backgroundColor: "#ffffff", borderWidth: "1px", borderStyle: "solid", borderColor: "var(--border)",
    borderRadius: "8px", color: "var(--foreground)", fontSize: "0.875rem",
    outline: "none", padding: "0.625rem 0.875rem", cursor: "pointer", textAlign: "left",
  };
  const errorButtonStyle: React.CSSProperties = { ...buttonStyle, borderColor: "var(--rose)", boxShadow: "0 0 0 3px rgba(225,29,72,0.1)" };

  const dropdownPanel: React.CSSProperties = {
    position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0,
    backgroundColor: "#fff", borderWidth: "1px", borderStyle: "solid", borderColor: "var(--border)",
    borderRadius: "8px", boxShadow: "0 8px 32px rgba(0,0,0,0.12)", zIndex: 60, overflow: "hidden",
  };
  ...
  if (loadingCountries) return <div className="flex items-center justify-center p-8"><p style={{ color: "var(--muted-foreground)" }}>Chargement des pays...</p></div>;
  if (errorCountries) return (
    <div className="flex flex-col items-center justify-center p-8 gap-4">
      <p style={{ color: "var(--rose)" }}>{errorCountries}</p>
      <button onClick={fetchCountries} className="px-4 py-2 rounded-lg text-sm" style={{ backgroundColor: "var(--primary)", color: "#fff", border: "none" }}>Réessayer</button>
    </div>
  );
      ...
      <div className="flex flex-col gap-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => { setForm({ ...form, senderName: e.target.value }); if (errors.senderName) setErrors({ ...errors, senderName: undefined }); }}
            onFocus={(e) => { if (!errors.senderName) { e.currentTarget.style.borderColor = focusStyle.borderColor; e.currentTarget.style.boxShadow = focusStyle.boxShadow; } }}
            onBlur={(e)  => { if (!errors.senderName) { e.currentTarget.style.borderColor = "var(--border)"; e.currentTarget.style.boxShadow = "none"; } }}
          />
          {errors.senderName && <p style={{ color: "var(--rose)", fontSize: "0.7rem", marginTop: 4 }}>{errors.senderName}</p>}
          <p style={{ color: "var(--muted-foreground)", fontSize: "0.7rem", marginTop: 4 }}>Max 11 caractères · Alphanumérique + underscore + espace autorisés</p>
        </div>

        {/* ── Pays (multi) ── */}
        <div className="relative">
          <Label required>Destinations (pays)</Label>
          <button
            style={errors.countries ? errorButtonStyle : buttonStyle}
            onClick={() => { closeAll(); setCountryOpen(!countryOpen); }}
          >
            <span style={{ color: "var(--muted-foreground)" }}>
              {form.countries.length === 0 ? "Sélectionner un ou plusieurs pays…" : `${form.countries.length} pays sélectionné(s)`}
            </span>
          ...
          {form.countries.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {selectedCountryObjs.map((c) => (
                <span key={c.code} className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs"
                  style={{ backgroundColor: "var(--primary)", color: "#fff", fontWeight: 500 }}>
                  {c.flag} {c.name}
                  <button onClick={() => toggleCountry(c.code)} style={{ background: "none", border: "none", cursor: "pointer", color: "#fff", padding: 0, lineHeight: 1 }}>
                    <X size={10} />
                  </button>
                </span>
              ))}
          ...
          {errors.countries && <p style={{ color: "var(--rose)", fontSize: "0.7rem", marginTop: 4 }}>{errors.countries}</p>}
            ...
            <div style={dropdownPanel}>
              <div className="flex items-center gap-2 px-3 py-2.5" style={{ borderBottom: "1px solid var(--border)" }}>
                ...
                <input autoFocus className="flex-1 bg-transparent outline-none"
                  ...
                  placeholder="Taper pour filtrer…" value={countrySearch}
                  onChange={(e) => setCountrySearch(e.target.value)} />
                {countrySearch && <button onClick={() => setCountrySearch("")}><X size={12} style={{ color: "var(--muted-foreground)" }} /></button>}
              </div>
              <div style={{ maxHeight: 220, overflowY: "auto" }}>
                {filteredCountries.map((c) => {
                  ...
                  return (
                    <button key={c.code}
                      className="w-full px-3.5 py-2.5 text-left flex items-center justify-between"
                      style={{ fontSize: "0.875rem", color: "var(--foreground)", background: selected ? "rgba(37,99,235,0.06)" : "transparent", border: "none", cursor: "pointer" }}
                      onMouseEnter={(e) => { if (!selected) (e.currentTarget as HTMLButtonElement).style.backgroundColor = "#f8fafc"; }}
                      onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.backgroundColor = selected ? "rgba(37,99,235,0.06)" : "transparent"; }}
                ...
                {filteredCountries.length === 0 && <p className="px-3.5 py-3" style={{ color: "var(--muted-foreground)", fontSize: "0.875rem" }}>Aucun résultat</p>}
              </div>
        ...
        {form.countries.length > 0 && (
          <div>
            <div className="flex items-center justify-between mb-1">
              <Label required>Opérateurs par pays</Label>
              <button type="button"
                onClick={() => { setAddOperatorTarget(form.countries[0]); setShowAddOperatorModal(true); }}
            ...
            {errors.operators && <p style={{ color: "var(--rose)", fontSize: "0.7rem", marginBottom: 6 }}>{errors.operators}</p>}

            <div className="flex flex-col gap-2">
              {selectedCountryObjs.map((countryObj) => {
                const selectedOps = form.operatorsByCountry[countryObj.code] ?? [];
                const allOps = countryObj.operators;
                const isExpanded = expandedCountry === countryObj.code;
                const hasNoOp = selectedOps.length === 0;
                const allSelected = allOps.length > 0 && allOps.every((op) => selectedOps.includes(op));

                return (
                  <div key={countryObj.code}
                    style={{
                      borderWidth: "1px", borderStyle: "solid",
                      borderColor: hasNoOp && errors.operators ? "var(--rose)" : "var(--border)",
                      borderRadius: "8px", overflow: "hidden",
                      boxShadow: hasNoOp && errors.operators ? "0 0 0 3px rgba(225,29,72,0.07)" : "none",
                    }}
                  >
                    {/* En-tête du pays */}
                    <button
                      className="w-full flex items-center justify-between px-3 py-2.5"
                      style={{ background: isExpanded ? "rgba(37,99,235,0.04)" : "#fafafa", border: "none", cursor: "pointer", textAlign: "left" }}
                      onClick={() => setExpandedCountry(isExpanded ? null : countryObj.code)}
                    >
                      <span className="flex items-center gap-2" style={{ fontWeight: 600, fontSize: "0.875rem", color: "var(--foreground)" }}>
                        {countryObj.flag} {countryObj.name}
                        <span className="px-1.5 py-0.5 rounded-full text-xs"
                          style={{ backgroundColor: selectedOps.length > 0 ? "var(--primary)" : "var(--secondary)", color: selectedOps.length > 0 ? "#fff" : "var(--muted-foreground)", fontWeight: 600 }}>
                      ...
                      <span className="flex items-center gap-2">
                        ...
                        {allOps.length > 0 && (
                          <span
                            role="button"
                            onClick={(e) => { e.stopPropagation(); selectAllOperatorsForCountry(countryObj.code); }}
                            style={{ fontSize: "0.7rem", color: "var(--primary)", fontWeight: 600, cursor: "pointer", padding: "2px 6px", borderRadius: 4, border: "1px solid var(--primary)" }}
                          >
                            {allSelected ? "Désélectionner tout" : "Tout sélectionner"}
                          </span>
                        )}
                        <ChevronRight size={14} style={{ color: "var(--muted-foreground)", transform: isExpanded ? "rotate(90deg)" : "none", transition: "transform 0.15s" }} />
                      </span>
                    </button>

                    {/* Liste des opérateurs (dépliable) */}
                    {isExpanded && (
                      <div style={{ borderTop: "1px solid var(--border)" }}>
                        {allOps.length === 0 ? (
                          <div className="px-4 py-3 flex items-center justify-between">
                            <p style={{ color: "var(--muted-foreground)", fontSize: "0.8rem" }}>⚡ Aucun opérateur prédéfini</p>
                            <button
                              onClick={() => { setAddOperatorTarget(countryObj.code); setShowAddOperatorModal(true); }}
                          ...
                          allOps.map((op) => {
                            ...
                            return (
                              <button key={op}
                                ...
                                style={{ fontSize: "0.875rem", color: "var(--foreground)", background: checked ? "rgba(37,99,235,0.05)" : "transparent", border: "none", borderBottom: "1px solid var(--border)", cursor: "pointer", textAlign: "left" }}
                                onMouseEnter={(e) => { if (!checked) (e.currentTarget as HTMLButtonElement).style.backgroundColor = "#f8fafc"; }}
                                onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.backgroundColor = checked ? "rgba(37,99,235,0.05)" : "transparent"; }}
                                onClick={() => toggleOperatorForCountry(countryObj.code, op)}
                              >
                                <span className="flex items-center gap-2.5">
                                  <span style={{
                                    width: 16, height: 16, borderRadius: 4, display: "inline-flex", alignItems: "center", justifyContent: "center",
                                    backgroundColor: checked ? "var(--primary)" : "#fff",
                                    borderWidth: "1.5px", borderStyle: "solid", borderColor: checked ? "var(--primary)" : "var(--border)",
                                    flexShrink: 0,
                                  }}>
                                    {checked && <svg width="10" height="10" viewBox="0 0 10 10"><polyline points="1.5,5 4,7.5 8.5,2" stroke="#fff" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                                  </span>
                                  {op}
                                </span>
                              </button>
                    ...
                    {/* Badges opérateurs sélectionnés (quand replié) */}
                    {!isExpanded && selectedOps.length > 0 && (
                      <div className="flex flex-wrap gap-1 px-3 pb-2.5" style={{ borderTop: "1px solid var(--border)", paddingTop: "8px" }}>
                        {selectedOps.map((op) => (
                          <span key={op} className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs"
                            style={{ backgroundColor: "rgba(37,99,235,0.1)", color: "var(--primary)", fontWeight: 500 }}>
                            {op}
                            <button onClick={(e) => { e.stopPropagation(); toggleOperatorForCountry(countryObj.code, op); }}
        ...
        {showAddOperatorModal && (
          <div className="fixed inset-0 flex items-center justify-center z-50" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
            <div className="rounded-xl p-6 w-96" style={{ backgroundColor: "#fff" }}>
              <h3 className="text-lg font-semibold mb-3">Ajouter un opérateur</h3>
              {form.countries.length > 1 && (
                <div className="mb-3">
                  <p className="text-sm mb-1" style={{ color: "var(--muted-foreground)" }}>Pour quel pays ?</p>
                  <select className="w-full px-3 py-2 rounded-lg" style={inputStyle}
                    value={addOperatorTarget} onChange={(e) => setAddOperatorTarget(e.target.value)}>
                    {selectedCountryObjs.map((c) => <option key={c.code} value={c.code}>{c.flag} {c.name}</option>)}
                  </select>
                </div>
              )}
              {form.countries.length === 1 && (
                <p className="text-sm mb-3" style={{ color: "var(--muted-foreground)" }}>
                  Pour : <strong>{selectedCountryObjs[0]?.name}</strong>
                </p>
              )}
              <input className="w-full px-3 py-2 rounded-lg mb-4" style={inputStyle}
                placeholder="Nom de l'opérateur" value={newOperatorName}
                onChange={(e) => setNewOperatorName(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") addNewOperator(); }} />
              <div className="flex gap-3">
                <button onClick={addNewOperator} className="flex-1 py-2 rounded-lg"
                  style={{ backgroundColor: "var(--primary)", color: "#fff", border: "none", cursor: "pointer" }}>Ajouter</button>
                <button onClick={() => { setShowAddOperatorModal(false); setNewOperatorName(""); }} className="flex-1 py-2 rounded-lg"
                  style={{ borderWidth: "1px", borderStyle: "solid", borderColor: "var(--border)", backgroundColor: "#fff", cursor: "pointer" }}>Annuler</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="relative">
          ...
          <button style={errors.status ? errorButtonStyle : buttonStyle} onClick={() => { closeAll(); setStatusOpen(!statusOpen); }}>
            {selectedStatus ? (
              <span className="flex items-center gap-2.5">
          ...
          {errors.status && <p style={{ color: "var(--rose)", fontSize: "0.7rem", marginTop: 4 }}>{errors.status}</p>}
          {statusOpen && (
            <div style={dropdownPanel}>
              {statusOptions.map((s) => (
                <button key={s.value}
                  className="w-full px-3.5 py-3 text-left flex items-center justify-between"
                  ...
                  onClick={() => { setForm({ ...form, status: s.value, comment: s.value !== "Rejected" ? "" : form.comment }); setStatusOpen(false); if (errors.status) setErrors({ ...errors, status: undefined }); }}
                >
        ...
        <div className="rounded-lg overflow-hidden transition-all duration-300"
          style={{ borderWidth: "1px", borderStyle: "solid", borderColor: isRejected ? "var(--rose)" : "var(--border)", backgroundColor: isRejected ? "var(--rose-muted)" : "#fafafa", opacity: isRejected ? 1 : 0.55 }}>
          <div className="px-4 pt-3 flex items-center justify-between">
            ...
            {!isRejected
              ? <span className="flex items-center gap-1.5 px-2 py-0.5 rounded-full" style={{ backgroundColor: "var(--secondary)", color: "var(--muted-foreground)", fontSize: "0.65rem" }}><Lock size={9} /> Activé uniquement si Rejeté</span>
              : <span className="flex items-center gap-1.5" style={{ color: "var(--rose)", fontSize: "0.75rem", fontWeight: 600 }}><AlertTriangle size={11} /> Champ obligatoire</span>}
          </div>
          <textarea disabled={!isRejected}
          ...
          />
          {errors.comment && isRejected && <p className="px-4 pb-3" style={{ color: "var(--rose)", fontSize: "0.7rem" }}>{errors.comment}</p>}
        </div>

        {/* ── Résumé des combinaisons ── */}
        {combinations.length > 0 && (
          <div className="rounded-lg p-3" style={{ backgroundColor: "rgba(37,99,235,0.05)", borderWidth: "1px", borderStyle: "solid", borderColor: "rgba(37,99,235,0.2)" }}>
            <p style={{ color: "var(--primary)", fontSize: "0.8125rem", fontWeight: 600, marginBottom: 6 }}>
              📋 {combinations.length} enregistrement(s) seront créés
            </p>
            {selectedCountryObjs.map((c) => {
              const ops = form.operatorsByCountry[c.code] ?? [];
              if (ops.length === 0) return null;
              return (
                <div key={c.code} style={{ marginBottom: 6 }}>
                  <p style={{ fontSize: "0.75rem", fontWeight: 600, color: "var(--foreground)", marginBottom: 2 }}>{c.flag} {c.name}</p>
                  {ops.map((op) => (
                    <p key={op} style={{ fontSize: "0.72rem", color: "var(--muted-foreground)", paddingLeft: 12 }}>
                      → {form.senderName || "…"} + {op} + {selectedStatus?.label ?? "…"}
                    </p>
                  ))}
                </div>
              );
      ...
      <>
        ...
        <div className="flex gap-3 mt-5">
          <button onClick={handleSubmit} disabled={submitting}
            ...
            onMouseLeave={(e) => { if (!submitting) (e.currentTarget as HTMLButtonElement).style.backgroundColor = "var(--primary)"; }}>
    ...
    <div className="rounded-xl p-6"
      ...
      <div className="flex gap-3 mt-6">
        ...
        <button onClick={handleReset}
          ...
          style={{ backgroundColor: "#f3f4f6", color: "#374151", border: "1px solid #e5e7eb", cursor: "pointer" }}
